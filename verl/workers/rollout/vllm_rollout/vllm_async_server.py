...
class vLLMHttpServerBase:
    ...
    async def generate(
        ...
        max_model_tokens = self.config.max_model_len - len(prompt_ids)
        requested_max_tokens = (
            sampling_params.pop("max_tokens", None)
            or sampling_params.pop("max_new_tokens", None)
            or self.config.response_length
        )
        max_tokens = min(requested_max_tokens, max_model_tokens)
